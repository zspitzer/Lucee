			...
			for (int i = 0; i < cols.length; i++) {
				...
				if (doSimpleTypes) {
					...
					if (value == null) prepStat.setNull(i + 1, types[i]);
						...
						prepStat.setTimestamp(i + 1, (value.equals("")) ? null : new Timestamp(DateCaster.toDateAdvanced(query.getAt(cols[i], y + 1), pc.getTimeZone()).getTime()));
					else if (type == DOUBLE) prepStat.setDouble(i + 1, (value.equals("")) ? 0 : Caster.toDoubleValue(query.getAt(cols[i], y + 1)));
					else if (type == INT) prepStat.setLong(i + 1, (value.equals("")) ? 0 : Caster.toLongValue(query.getAt(cols[i], y + 1)));
			...
			try {
				...
				try {
					...
					while (it.hasNext()) {
						...
						addTable(conn, pc, modTableName, Caster.toQuery(pc.getVariable(StringUtil.removeQuotes(tableName, true))), doSimpleTypes, usedTables); // strip any double quotes around table names
					}
					...
					DatabaseException de = new DatabaseException("Error executing SQL statement on query", null, sql, null);

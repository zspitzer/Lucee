...
public abstract class ComponentPageImpl extends ComponentPage implements PagePro {
	...
	public Object call(PageContext pc) throws PageException {
		...
		try {
            ...
            if(path.size()>1 && !(path.size()==3 && ListUtil.last(path.getE(2).toString(),"/\\",true)
            		.equalsIgnoreCase(pc.getRequestDialect()==CFMLEngine.DIALECT_CFML?lucee.runtime.config.Constants.CFML_APPLICATION_EVENT_HANDLER:lucee.runtime.config.Constants.LUCEE_APPLICATION_EVENT_HANDLER)) ) {// MUSTMUST bad impl -> check with and without application . cfc
	...
	private void callWSDL(PageContext pc, Component component) throws ServletException, IOException, ExpressionException {
    	...
    	// create a wsdl file
	    	...
	    	RPCServer.getInstance(pc.getId(),pc,pc.getServletContext())

...
public final class ScopeContext {
	...
	public static void info(Log log, String msg) {
		if(log != null)
			log.log(Log.LEVEL_INFO, "scope-context", msg);
	}

	public static void error(Log log, String msg) {
		if(log != null)
			log.log(Log.LEVEL_ERROR, "scope-context", msg);
	}

	public static void error(Log log, Throwable t) {
		if(log != null)
			log.log(Log.LEVEL_ERROR, "scope-context", ExceptionUtil.getStacktrace(t, true));
	}
	...
	/**
	 * Returns the current Cluster Scope, if there is no current Cluster Scope and create is true, returns a new Cluster Scope. If create is false and the
	 * request has no valid Cluster Scope, this method returns null.
	 *
	 * @param config
	 * @param create
	 * @return
			...
			if("file".equals(storage)) {
				...
				if(ds != null) {
					if(INVIDUAL_STORAGE_KEYS) {
						try {
							client = (Client)IKStorageScopeSupport.getInstance(Scope.SCOPE_CLIENT, new IKHandlerDatasource(), appContext.getName(), storage, pc,
									existing, getLog());
						}
					...
					if(INVIDUAL_STORAGE_KEYS) {
						try {
							client = (Client)IKStorageScopeSupport.getInstance(Scope.SCOPE_CLIENT, new IKHandlerCache(), appContext.getName(), storage, pc,
									existing, getLog());
						}
						catch (PageException pe) {
							// code above could fail when an old scope is loaded, remember client scope can be easy be
				...
				if(client == null) {
					...
					if(cc != null)
						throw new ApplicationException("cache [" + storage
								+ "] is not enabled to be used  as a session/client storage, you have to enable it in the Lucee administrator.");
	 ...
	 *
	 * client=(ClientPlus) context.get(pc.getCFID()); if(client==null || client.isExpired() || !client.getStorageType().equalsIgnoreCase(storage)) {
	 * if(StringUtil.isEmpty(storage) || "file".equals(storage) || "registry".equals(storage)){ storage="file";
	 * client=ClientFile.getInstance(appContext.getName(),pc,getLog()); } else if("cookie".equals(storage))
	 * client=ClientCookie.getInstance(appContext.getName(),pc,getLog()); else if("memory".equals(storage) || "ram".equals(storage)){ //storage="ram";
	 * client=ClientMemory.getInstance(pc,getLog()); } else{ DataSource ds = ((ConfigImpl)pc.getConfig()).getDataSource(storage,null);
	 * if(ds!=null)client=ClientDatasource.getInstanceEL(storage,pc,getLog()); else client=ClientCache.getInstanceEL(storage,appContext.getName(),pc,getLog());
	 *
	 * } client.setStorage(storage); context.put(pc.getCFID(),client); } else getLog().info("scope-context",
	 * "use existing client scope for "+appContext.getName()+"/"+pc.getCFID()+" from storage "+storage);
	 *
	 *
	...
	public boolean hasExistingSessionScope(PageContext pc) {
		if(pc.getSessionType() == Config.SESSION_TYPE_APPLICATION)
			return hasExistingCFSessionScope(pc);
	...
	public boolean hasExistingCFID(PageContext pc, String cfid) {
		if(hasExistingCFSessionScope(pc, cfid))
			return true;
	...
	private Session getCFSessionScope(PageContext pc, RefBoolean isNew) throws PageException {
		...
		if(existing != null && (existing.isExpired() || !(existing instanceof StorageScope)))
			existing = null; // second should not happen
		...
		if(session == null || !(session instanceof StorageScope) || !((StorageScope)session).getStorage().equalsIgnoreCase(storage)) {
			...
			if(isMemory) {
				...
				if(ds != null && ds.isStorage()) {
					if(INVIDUAL_STORAGE_KEYS) {
						try {
							session = (Session)IKStorageScopeSupport.getInstance(Scope.SCOPE_SESSION, new IKHandlerDatasource(), appContext.getName(), storage,
									pc, existing, getLog());
						}
						catch (PageException pe) {
					...
					if(INVIDUAL_STORAGE_KEYS) {
						try {
							session = (Session)IKStorageScopeSupport.getInstance(Scope.SCOPE_SESSION, new IKHandlerCache(), appContext.getName(), storage, pc,
									existing, getLog());
						}
						catch (PageException pe) {
			...
			getLog().log(Log.LEVEL_INFO, "scope-context",
					"use existing session scope for " + appContext.getName() + "/" + pc.getCFID() + " from storage " + storage);
		}
	...
	public void clearUnused() {
		...
		try {
			...
			if(session == null)
				session = new StorageScopeEngine(factory, log, new StorageScopeCleaner[] { new FileStorageScopeCleaner(Scope.SCOPE_SESSION, null)// new
						// SessionEndListener())
			...
			if(client == null)
				client = new StorageScopeEngine(factory, log, new StorageScopeCleaner[] { new FileStorageScopeCleaner(Scope.SCOPE_CLIENT, null),
	...
	private static void migrate(PageContextImpl pc, UserScope oldScope, UserScope newScope, boolean migrate) {
		...
		if(migrate) {
			while(it.hasNext()) {
				e = it.next();
				if(StorageScopeImpl.KEYS.contains(e.getKey()))
					continue;

...
public class QueryImpl implements Query, Objects, QueryResult {
	...
	public QueryImpl(PageContext pc, DatasourceConnection dc, SQL sql, int maxrow, int fetchsize, TimeSpan timeout, String name, String template)
			throws PageException {
	...
	public static QueryStruct toStruct(PageContext pc, DatasourceConnection dc, SQL sql, Collection.Key keyName, int maxrow, int fetchsize, TimeSpan timeout,
			String name, String template, boolean createUpdateData, boolean allowToCachePreperadeStatement) throws PageException {
	...
	public static QueryArray toArray(PageContext pc, DatasourceConnection dc, SQL sql, int maxrow, int fetchsize, TimeSpan timeout, String name,
	...
	private static void setItems(PageContext pc, TimeZone tz, PreparedStatement preStat, SQLItem[] items)
			throws DatabaseException, PageException, SQLException {
	...
	private static boolean fillResult(QueryImpl qry, QueryResult qr, Collection.Key keyName, DatasourceConnection dc, ResultSet result, int maxrow,
	...
	private static int getIndexFrom(Collection.Key[] tmpColumnNames, Collection.Key key, int from, int to) {
		for (int i = from; i < to; i++) {
			if(tmpColumnNames[i] != null && tmpColumnNames[i].equalsIgnoreCase(key))
				return i;
		}
		return -1;
	}
	...
	public QueryImpl(String[] strColumns, String[] strTypes, int rowNumber, String name) throws DatabaseException {
		this.name = name;
		columncount = strColumns.length;
		if(strTypes.length != columncount)
	 ...
	 * 
	 * @param columnNames
	 *            columns definition as String Array
	 * @param arrColumns
	 *            values
	 * @param name
	 * @throws DatabaseException
	...
	private static void validateColumnNames(Key[] columnNames) throws DatabaseException {
		...
		for (int i = 0; i < columnNames.length; i++) {
			...
			// throw new DatabaseException("invalid column name ["+columnNames[i]+"] for query", "column names must start with a letter and can be followed by
			...
			if(testMap.contains(columnNames[i].getLowerString()))
				throw new DatabaseException("invalid parameter for query, ambiguous column name " + columnNames[i],
	 ...
	 * this.columnNames=columnNames; this.columns=columns; this.exeTime=exeTime; this.isCached=isCached; this.name=name; this.columncount=columnNames.length;
	 * this.recordcount=columns.length==0?0:columns[0].size(); this.sql=sql;
	 * 
	 * }
	 */
	...
	public QueryImpl(Collection.Key[] columnNames, Array[] arrColumns, String name) throws DatabaseException {
		...
		if(columnNames.length != arrColumns.length)
			throw new DatabaseException("invalid parameter for query, not the same count from names and columns",
		...
		if(arrColumns.length > 0) {
			...
			for (int i = 0; i < arrColumns.length; i++) {
				if(arrColumns[i].size() != len)
					throw new DatabaseException("invalid parameter for query, all columns must have the same size",
	...
	public Object getAt(Collection.Key key, int row) throws PageException {
		...
		throw new DatabaseException("column [" + key + "] not found in query, columns are [" + getColumnlist(getKeyCase(ThreadLocalPageContext.get())) + "]",
				null, sql, null);
	}
	...
	public QueryColumn removeColumn(Collection.Key key) throws DatabaseException {
		...
		if(removed == null) {
			if(key.equals(KeyConstants._RECORDCOUNT) || key.equals(KeyConstants._CURRENTROW) || key.equals(KeyConstants._COLUMNLIST))
				throw new DatabaseException("can't remove " + key + " this is not a row",
	...
	public Object setAt(Collection.Key key, int row, Object value) throws PageException {
		...
		throw new DatabaseException("column [" + key + "] does not exist", "columns are [" + getColumnlist(getKeyCase(ThreadLocalPageContext.get())) + "]", sql,
				null);
	}
	...
	private void sort(QueryColumn column, int order) throws PageException {
		...
		Arrays.sort(arr, (type == Types.BIGINT || type == Types.BIT || type == Types.INTEGER || type == Types.SMALLINT || type == Types.TINYINT
				|| type == Types.DECIMAL || type == Types.DOUBLE || type == Types.NUMERIC || type == Types.REAL) ?

						(Comparator)new NumberSortRegisterComparator(order == ORDER_ASC)
						: (Comparator)new SortRegisterComparator(null, order == ORDER_ASC, true, true));
	...
	/*
	 * * if this query is still connected with cache (same query also in cache) it will disconnetd from cache (clone object and add clone to cache)
	 */
	...
	public QueryImpl cloneQuery(boolean deepCopy) {
		...
		try {
			...
			if(generatedKeys != null)
				newResult.generatedKeys = generatedKeys.cloneQuery(false);
			return newResult;
		} finally {
			if(!inside)
	...
	public QueryColumn getColumn(Collection.Key key) throws DatabaseException {
		...
		if(key.length() >= 10) {
			if(key.equals(KeyConstants._RECORDCOUNT))
				return new QueryColumnRef(this, key, Types.INTEGER);
			if(key.equals(KeyConstants._CURRENTROW))
				return new QueryColumnRef(this, key, Types.INTEGER);
			if(key.equals(KeyConstants._COLUMNLIST))
				return new QueryColumnRef(this, key, Types.INTEGER);
		...
		throw new DatabaseException(
				"key [" + key.getString() + "] not found in query, columns are [" + getColumnlist(getKeyCase(ThreadLocalPageContext.get())) + "]", null, sql,
				null);
	}
			...
			for (int y = 0; y < keys.length; y++) {
				try {
					...
					if(o instanceof String)
						...
						sb.append(getToStringField(Caster.toString(((Number)o))));
	...
	private int getIndexFromKey(String key) {
		...
		for (int i = 0; i < columnNames.length; i++) {
			if(columnNames[i].getLowerString().equals(lc))
				return i;
		}
		return -1;
	}
	...
	public int getColumnIndex(String coulmnName) {
		Collection.Key[] keys = keys();
		for (int i = 0; i < keys.length; i++) {
			if(keys[i].getString().equalsIgnoreCase(coulmnName))
				return i + 1;
		}
		return -1;
	}
	...
	public Object getObject(int columnIndex) throws SQLException {
		if(columnIndex > 0 && columnIndex <= columncount)
			return getObject(this.columnNames[columnIndex - 1].getString());
		return null;
	}
	...
	public String getString(int columnIndex) throws SQLException {
		...
		if(rtn == null)
			return null;
		if(Decision.isCastableToString(rtn))
			return Caster.toString(rtn, null);
	...
	public boolean getBoolean(int columnIndex) throws SQLException {
		...
		if(rtn == null)
			return false;
		if(Decision.isCastableToBoolean(rtn))
			return Caster.toBooleanValue(rtn, false);
	...
	public boolean absolute(int row) throws SQLException {
		if(recordcount == 0) {
			if(row != 0)
				throw new SQLException("invalid row [" + row + "], query is Empty");
			return false;
		}
		// row=row%recordcount;
	...
	public int findColumn(String columnName) throws SQLException {
		...
		if(index == -1)
			throw new SQLException("invald column definitions [" + columnName + "]");
		return index;
	}
	...
	public static QueryImpl cloneQuery(Query qry, boolean deepCopy) {
		...
		try {
			...
			if(qry.getGeneratedKeys() != null)
				newResult.generatedKeys = ((QueryImpl)qry.getGeneratedKeys()).cloneQuery(false);
			return newResult;
		} finally {
			if(!inside)
				ThreadLocalDuplication.reset();

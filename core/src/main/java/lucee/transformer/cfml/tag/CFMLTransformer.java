...
public final class CFMLTransformer {
	...
	public Page transform(Factory factory,ConfigImpl config,SourceCode sc,TagLib[] tlibs,FunctionLib[] flibs, long sourceLastModified, Boolean dotNotationUpperCase, boolean returnValue, boolean ignoreScope) throws TemplateException {
		...
		Page page=new Page(factory,config,sc,null,
		...
		try {
			do {
				...
				if(data.srcCode.isAfterLast()) break;
				if(data.srcCode.forwardIfCurrent("</")){
					...
					if(tagLib==null){
						...
						else throw new TemplateException(sc,"no matching start tag for end tag ["+tagLib.getNameSpaceAndSeparator()+name+"]");
	 ...
	 * @param parseExpression Definiert ob Expressions innerhalb von Literalen uebersetzt werden sollen oder nicht.
...
//		TODO muss erlaubt sein
				...
				while(true)	{
					...
					if(tagLibEnd!=null)	{
					    ...
					    if(tagLibTag.isBodyReq()) {
							...
							else throw new TemplateException(data.srcCode,
			...
			if(type==TagLibTag.ATTRIBUTE_TYPE_FIXED || type==TagLibTag.ATTRIBUTE_TYPE_MIXED)	{
				...
				while(it.hasNext())	{
					...
					if(att.isRequired() && !contains(args,att) && att.getDefaultValue()==null)	{
						if(!hasAttributeCollection)throw createTemplateException(data.srcCode,"attribute "+att.getName()+" is required for tag "+tag.getFullName(),tag);
	 ...
	 * @throws TemplateException
	 */
	private static String attributeName(SourceCode cfml,RefBoolean dynamic, ArrayList<String> args, TagLibTag tag, 
